...
function UpdateEntry({entry,onUpdate, onClose}:props) {
    const {user}=useAuth();
    ...
    const handleUpdate= async(e:FormEvent)=>{
        ...
        try{
             const { data } = await api.put<JournalEntry>(`/journals/${entry.id}`, {"title":title, "description":description,"updated_at":new Date()});
            onUpdate(data);
            queryClient.setQueryData<JournalEntry[]>(['journals'], (old)=> old ? old.map((j)=> j.id === data.id ? data : j) : old);
  ...
  return (
    <div className='fixed inset-0 w-full bg-black/50 bg-blend-color-burn z-10 min-h-dvh'>
        ...
        w-[40%] bg-amber-200  p-[1%] space-y-[0.5%] dark:bg-stone-900 dark:border-amber-500 dark:shadow-[0_0_17px] dark:shadow-amber-500'>
            ...
            <div className='flex flex-row items-center space-x-[2%] relative dark:text-amber-400'>
                <Feather /> 
                <h1 className=' text-2xl '>Update Your Entry</h1>
            ...
            <p className=' mb-[6%] italic text-amber-800 dark:text-amber-100'>Keep your personal chronicle up to date</p>

            {/** Form Body*/}
            <form onSubmit={handleUpdate} onChange={()=>setError("")}  className=' flex flex-col relative'>
